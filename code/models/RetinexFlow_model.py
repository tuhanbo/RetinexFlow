...
class RetinexFlowModel(BaseModel):
    def __init__(self, opt, step):
        super(RetinexFlowModel, self).__init__(opt)
        self.opt = opt
        ...
        for k, v in self.netG.named_parameters():  # can optimize for a part of the model
            if v.requires_grad:
                if 'RRDB.' in k:
                    ...
                    # for param in optim_params_RRDB:
            ...
            for optimizer in self.optimizers:
                self.schedulers.append(
                    lr_scheduler.CosineAnnealingRestartCyclicLR(
                        optimizer, **train_opt['scheduler_CosineAnnealingRestartCyclicLR']))
        else:
    ...
    def feed_data(self, data, need_GT=True):
        ...
        # self.embed = data['SAM_embed'].to(self.device)
        # if data['SEGMENT'] is not None:
        #     self.segment = data['SEGMENT'].to(self.device) # segment
        if need_GT:
    ...
    def optimize_parameters(self, step):
        ...
        weight_ll = opt_get(self.opt, ['train', 'weight_ll'])
        weight_fl = 1 if weight_fl is None else weight_fl
        weight_ll = 1 if weight_ll is None else weight_ll
        weight_l1 = opt_get(self.opt, ['train', 'weight_l1']) or 0
        ...
        if weight_fl > 0:
            if self.opt['optimize_all_z']:
                ...
                epses = None
            z, nll, loss_Decom, y_logits = self.netG(gt=self.real_H, lr=self.var_L, reverse=False,epses=epses,
                                         align_condition_feature=opt_get(self.opt,
                                                                         ['align_condition_feature']) or False)
            nll_loss = torch.mean(nll)
            losses['nll_loss'] = nll_loss * weight_fl
            loss_Decom = torch.mean(loss_Decom)
            losses['Decom_loss'] = loss_Decom * weight_ll
        ...
        if not self.already_print_params_num:
            logger.info("Parameters with grad of full network %.4f and encoder %.4f"%(sum([m.numel() for m in self.netG.parameters() if m.grad is not None])/1e6, sum([m.numel() for m in self.netG.RRDB.parameters() if m.grad is not None])/1e6))
        ...
        mean = total_loss.item()
        return mean, nll_loss, loss_Decom
    ...
    def test(self):
        ...
        if self.heats is not None:
            ...
            with torch.no_grad():
                ...
                self.fake_H[(0, 0)], logdet, self.lr_reflectogram, self.gt_reflectogram = self.netG(lr=self.var_L, gt=self.real_H,
                                                                                                    z=z.to(self.var_L.device), eps_std=0, reverse=True)
    ...
    def get_sr_with_z(self, lq, heat=None, seed=None, z=None, epses=None):
        ...
        with torch.no_grad():
            sr, logdet, lr_reflectogram, gt_reflectogram = self.netG(lr=lq, z=z, eps_std=heat, reverse=True, epses=epses)
        self.netG.train()
        return sr, z
    ...
    def get_current_visuals(self, need_GT=True):
        ...
        if self.heats is not None:
            ...
            out_dict['gt_reflectogram'] = self.gt_reflectogram.detach()[0].float().cpu()
            out_dict['lr_reflectogram'] = self.lr_reflectogram.detach()[0].float().cpu()
            # out_dict['SEGMENT'] = self.out_segment.detach()[0].float().cpu()
        if need_GT:
