...
class CondAffineSeparatedAndCond(nn.Module):
    def __init__(self, in_channels, opt):
        ...
        self.a = self.predict_a(in_channels=self.in_channels_rrdb,
                                out_channels=self.out_channels_a,
                                hidden_channels=self.hidden_channels)
    ...
    def forward(self, input: torch.Tensor, logdet=None, reverse=False, ft=None):
        if not reverse:
            ...
            scale, shift = self.feature_extract_aff_new(z1, ft, self.fAffine, self.a)
    ...
    def feature_extract_aff_new(self, z1, ft, f1, f2):
        # z = torch.cat([z1, ft], dim=1)
        # clip the input features into range of [0,1]
        a = f2(ft)
        h = f1(z1)
        h = self.relu(h) - self.relu(h - 1)
        h = h + a * h * (1 - h)
        # for i in range(self.out_channels_a):
        #     h = h + a[:, i:i + 1] * h * (1 - h)
        shift, scale = thops.split_feature(h, "cross")
        scale = (torch.sigmoid(scale + 2.) + self.affine_eps)
        return scale, shift
    ...
    def predict_a(self, in_channels, out_channels, hidden_channels):
        return nn.Sequential(
            nn.Conv2d(in_channels, hidden_channels, 5, stride=1, padding=2), nn.ReLU(inplace=False),
            nn.Conv2d(hidden_channels, hidden_channels, 3, stride=1, padding=1), nn.ReLU(inplace=False),
            nn.Conv2d(hidden_channels, out_channels, 1, stride=1, padding=0),
            nn.Sigmoid()
        )