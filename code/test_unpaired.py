...
def main():
    ...
    for sub_lr_dir in os.listdir(lr_dir):
        sub_path = os.path.join(lr_dir, sub_lr_dir)
        if not os.path.isdir(sub_path):
            continue
        lr_paths = fiFindByWildcard(os.path.join(sub_path, '*.*'))
        this_dir = os.path.dirname(os.path.realpath(__file__))
        test_dir = os.path.join(this_dir, 'unpaired_results', conf, sub_lr_dir)
        os.makedirs(test_dir, exist_ok=True)

        print(f"Out dir: {test_dir}")

        for lr_path, idx_test in tqdm.tqdm(zip(lr_paths, range(len(lr_paths)))):
        # for lr_path, idx_test in tqdm.tqdm(enumerate(lr_paths), total=len(lr_paths)):
