...
from data.LoL_dataset import LoL_Dataset, LoL_Dataset_v2, RELLISUR_Dataset
...
def main():
    ...
    elif opt['dataset'] == 'RELLISUR_Dataset':
        dataset_cls = RELLISUR_Dataset
    else:
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step<2:
                mean, nll_loss, loss_Decom = 0, 0, 0
            else:
                mean, nll_loss, loss_Decom = model.optimize_parameters(current_step)
            ...
            if current_step % opt['logger']['print_freq'] == 0 \
                ...
                message = '<epoch:{:3d}, iter:{:8,d}, lr:{:.3e}, t:{:.2e}, td:{:.2e}, eta:{:.2e}, mean:{:.3e}, nll_loss:{:.3e}, loss_Decom:{:.3e}> '.format(
                    epoch, current_step, model.get_current_learning_rate(), avg_time, avg_data_time,
                    eta(avg_time), mean, nll_loss, loss_Decom)
                print(message)
            timer.tick()
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0:
                ...
                for val_data in val_loader:
                    ...
                    if hasattr(model, 'heats') and model.heats is not None:
                        ...
                        lr_reflectogram_img = util.tensor2img(visuals['lr_reflectogram'])
                        gt_reflectogram_img = util.tensor2img(visuals['gt_reflectogram'])

                        save_lr_reflectogram_img_path = os.path.join(img_dir,
                                                                     '{:s}_{:d}_lr_reflectogram.png'.format(img_name,
                                                                                                            current_step))
                        save_gt_reflectogram_img_path = os.path.join(img_dir,
                                                                     '{:s}_{:d}_gt_reflectogram.png'.format(img_name,
                                                                                                            current_step))
                    ...
                    assert lr_reflectogram_img is not None
                    assert gt_reflectogram_img is not None
                    ...
                    util.save_img(np.clip(lr_reflectogram_img, 0, 255).astype(np.uint8), save_lr_reflectogram_img_path)
                    util.save_img(np.clip(gt_reflectogram_img, 0, 255).astype(np.uint8), save_gt_reflectogram_img_path)
                    avg_psnr += psnr
